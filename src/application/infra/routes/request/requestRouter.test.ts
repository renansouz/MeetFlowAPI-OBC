...
const requestBody = {
  ...
  scheduleId: "61d83267240e5e3463a73e89",
...
describe("Route api/request", () => {
  ...
  describe("POST /api/request/add", () => {
    test("Should return 200 on add", async () => {
      ...
      expect(responseBodyAdd?.scheduleId).toBe(_id.toString());
      ...
      expect(responseBodyAdd?.status).toBe("solicitado");
    });
  ...
  describe("PATCH /api/request/update", () => {
    ...
    test("Should return 200 on update", async () => {
      ...
      const response = await fastify.inject({
        ...
        headers: { authorization: `Bearer ${token}` },
        body: { endDate: requestUpdateBody.endDate, status: "confirmado" },
      });
      const responseBody = JSON.parse(response.body);
      expect(response.statusCode).toBe(200);
