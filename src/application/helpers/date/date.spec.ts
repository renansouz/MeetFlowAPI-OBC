...
describe("date tests business rules", () => {
  ...
  test("mapBusinessHours function when i pass null as infoSchedule", async () => {
    expect(
      mapBusinessHours({ ...mockBusinessHoursInput, infoSchedule: null as any })
    ).toBeNull();
  });
  ...
  test("getArrayTimes function with have`t lunch time", async () => {
    expect(
      getArrayTimes({
        ...mockGetArrayTimes,
        infoSchedule: {
          ...mockGetArrayTimes.infoSchedule,
          hourLunchStart1: null,
          hourLunchEnd1: null,
        },
      })
  ...
  test("getArrayTimes function when i pass infoSchedule null", async () => {
    expect(getArrayTimes({ ...mockGetArrayTimes, infoSchedule: null as any })).toEqual({
      timeAvailable: [],
      timeAvailableProfessional: [],
    });
  });
